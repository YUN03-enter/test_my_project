#from yolo5.model_load import load_yolov5_model
...
from ControlComImageComposition.ldm.util import instantiate_from_config
from ControlComImageComposition.ldm.models.diffusion.ddim import DDIMSampler
from ControlComImageComposition.ldm.models.diffusion.plms import PLMSSampler
from torchvision.transforms import Resize
from ControlComImageComposition.ldm.data.open_images_control import get_tensor, get_tensor_clip, get_bbox_tensor, bbox2mask, mask2bbox
...
def load_yolov5_model() :
    ...
    #커스텀 모델 사용하는 경우
    #model = torch.hub.load('ultralytics/yolov5', 'custom', "파인튜닝된 pt파일 경로")
    return model

def detect_and_crop_people(background_path, image_path, output_dir):
    # 모델 불러오기
...
if __name__ == "__main__":
    ...
    background_image = "results/background"
    output_dir = "results/bbox"
    file_names = os.listdir(background_image)
    for file_name in file_names :
        detect_and_crop_people(background_image, file_name, output_dir)

    opt = argument_parse()
    task = 'composition'
    # if opt.task in ['blending', 'harmonization']:
    #     ckpt_dir = "/ControlComImageComposition/checkpoints"
    #     weight_path = ckpt_dir + '/ControlCom_blend_harm.pth'
    #     indicator   = [[0,0]] if opt.task == 'blending' else [[1,0]]
    # else:
    #     weight_path = ckpt_dir + '/ControlCom_view_comp.pth'
    #     indicator   = [[0,1]] if opt.task == 'viewsynthesis' else [[1,1]]
    # #assert os.path.exists(weight_path), weight_path 
    ckpt_dir = "ControlComImageComposition/checkpoints"
    weight_path = "ControlComImageComposition/checkpoints/ControlCom_view_comp.pth"
    indicator = [[1,1]]
    opt.config = 'ControlComImageComposition/configs/controlcom.yaml'
    config      = OmegaConf.load(opt.config)
    clip_path   = ckpt_dir +'/openai-clip-vit-large-patch14'
    ...
    for img_name in test_list:
        ...
        for i in range(comp_img.shape[0]):
            if i > 0:
                res_path = os.path.join(opt.outdir, img_name.split('.')[0] + f'_{task}_sample{i}.png')
            else:
                res_path = os.path.join(opt.outdir, img_name.split('.')[0] + f'_{task}.jpg')
            save_image(comp_img[i], res_path)
            print('save result to {}'.format(res_path))
        # if not opt.skip_grid:
        #     grid_img  = generate_image_grid(batch, comp_img)
        #     grid_path = os.path.join(opt.outdir, img_name.split('.')[0] + f'_{task}_grid.jpg')
